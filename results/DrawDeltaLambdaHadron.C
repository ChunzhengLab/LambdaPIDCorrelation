...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  dummyDelta->GetYaxis()->SetTitle("#LTcos(#phi_{#Lambda} - #phi_{2})");
  TLegend* legendDelta = new TLegend(0.15,0.55,0.45,0.85);
  legendDelta->AddEntry(hDelta_Lambda_hPos,"#Lambda - h^{+}","lp");
  legendDelta->AddEntry(hDelta_Lambda_hNeg,"#Lambda - h^{-}","lp");
  legendDelta->AddEntry(hDelta_AntiLambda_hNeg,"#bar{#Lambda} - h^{-}","lp");
  legendDelta->AddEntry(hDelta_AntiLambda_hPos,"#bar{#Lambda} - h^{+}","lp");
  legendDelta->AddEntry(hDelta_SS,"#Lambda - h^{+} + #bar{#Lambda} - h^{-}","lp");
  legendDelta->AddEntry(hDelta_OS,"#Lambda - h^{-} + #bar{#Lambda} - h^{+}","lp");


  TH2D* dummyDeltaDelta = new TH2D("","",1,0.,80.,1,-0.3e-3,0.3e-3);
  dummyDeltaDelta->GetXaxis()->SetTitle("centrality");
  dummyDeltaDelta->GetYaxis()->SetTitle("OS-SS");
  ...
  cDelta->SaveAs("DeltaLambdaHadron.pdf");
}
