 ...
 **************************************************************************/
/* $Id: AliAnalysisTaskMyTask.cxx ver: 2.0                     $   */
...
void AliAnalysisTaskMyTask::UserExec(Option_t*) {
  // eventCount++;
  // cout<<"we are in event    "<<eventCount<<endl;
...
///Track variables:
    ...
    for (Int_t iV0 = 0; iV0 < nV0s; iV0++) {
      ...
      if (code == 3122) {
        ...
        if (TMath::Abs(massLambda - fMassMean) < fLambdaMassCut) {
          ...
          // cout<<"V0 Pos ID    "<<id_posDaughter<<"      "<<phi<<endl;
          // cout<<"V0 Neg ID    "<<id_negDaughter<<"      "<<phi<<endl;
        }
      } 
  ...
  // cout<<"tracks ID"<<"       "<<"tracks NUA"<<"       "<<"tracks Phi"<<endl;
  // cout<<vecID.size()<<"       "<<vecNUAWeight.size()<<"       "<<vecPhi.size()<<endl;
  // for(int i = 0; i < vecID.size(); ++i) cout << vecID[i] <<"        "<< vecNUAWeight[i]<<"        "<<vecPhi[i]<<endl;
  ...
  // cout<<"size Pos EP"<<vecPosEPTrkID.size()<<endl;
  // for(int i = 0; i < vecPosEPTrkID.size(); ++i) cout << vecPosEPTrkID[i]<<endl;
  ...
  // cout<<"size Neg EP"<<vecNegEPTrkID.size()<<endl;
  // for(int i = 0; i < vecNegEPTrkID.size(); ++i) cout << vecNegEPTrkID[i]<<endl;

// }else return;

  //cout<<"a event pass"<<endl;
...
void AliAnalysisTaskMyTask::SetupAnalysisHistograms(){
...
void AliAnalysisTaskMyTask::SetupQAHistograms(){
...
Bool_t AliAnalysisTaskMyTask::IsGoodV0(AliAODv0 *aodV0) 
{
  if (!aodV0) {
...
Bool_t AliAnalysisTaskMyTask::IsGoodDaughterTrack(const AliAODTrack *track)
{
  // TPC refit
...
Int_t AliAnalysisTaskMyTask::GetLambdaCode(const AliAODTrack *pTrack, const AliAODTrack *nTrack) 
{
...
void AliAnalysisTaskMyTask::SetupPileUpRemovalFunctions(){
...
void AliAnalysisTaskMyTask::SetupEventAndTaskConfigInfo(){
...
void AliAnalysisTaskMyTask::SetupLambdaPIDFlowHistograms() {
//TODO
...
void AliAnalysisTaskMyTask::GetNUACorrectionHist(Int_t run, Int_t kParticleID) {
  if(fListNUACorr){
    //charge:
...
void AliAnalysisTaskMyTask::GetMCCorrectionHist(){
...
void AliAnalysisTaskMyTask::GetV0MCorrectionHist(Int_t run){ 
...
Bool_t AliAnalysisTaskMyTask::CheckPIDofParticle(AliAODTrack* ftrack,Int_t pidToCheck){
...
Bool_t AliAnalysisTaskMyTask::CheckEventIsPileUp2018(AliAODEvent *faod) {
...
Bool_t AliAnalysisTaskMyTask::GetTPCQvectAndRemovePileUp2018(AliAODEvent *faod,Double_t *qnxEtaNeg,Double_t *qnyEtaNeg,Double_t *qnxEtaPos,Double_t *qnyEtaPos,Double_t& multNeg,Double_t& multPos)
{
  Bool_t BisPileup=kFALSE;
  Double_t centrV0M=-99.0;
  Double_t centrCL1=-99.0;
  ...
  for (Int_t it = 0; it < nTracks; it++) {
    ...
    if(trk1Eta >= fEtaGapPosforEP){
      ...
      //cout<<"ID TPC Pos   "<<trk1ID<<"     "<<trk1Wgt<<"     "<<trk1Phi<<endl;
    }
      ...
      //cout<<"ID TPC Neg   "<<trk1ID<<"     "<<trk1Wgt<<"     "<<trk1Phi<<endl;
    }
...
Double_t AliAnalysisTaskMyTask::GetNUAWeightForTrack(Double_t fVtxZ,Double_t fPhi,Double_t fEta,Int_t gChrg){
...
Double_t AliAnalysisTaskMyTask::GetNUAWeightForTrackPID(Double_t fVtxZ,Double_t fPhi,Double_t fEta,Int_t gChrg){
...
void AliAnalysisTaskMyTask::ApplyTPCqVectRecenter(Float_t fCent,Int_t gPsiN,Double_t& qxEtaNeg, Double_t& qyEtaNeg,Double_t& qxEtaPos,Double_t& qyEtaPos){
...
void  AliAnalysisTaskMyTask::ApplyV0XqVectRecenter(Float_t fCent,Int_t gPsiN,Double_t &qnxV0C,Double_t &qnyV0C,Double_t &qnxV0A,Double_t &qnyV0A){
...
Double_t AliAnalysisTaskMyTask::GetMCEfficiencyWeightForTrack(Double_t fPt,Int_t gChrg,Int_t kPID){
  Int_t    iBinforNUE = 1;
...
Bool_t AliAnalysisTaskMyTask::GetGainCorrectedV0Qvector(AliAODEvent *faod,Double_t fVtxZ,Int_t gPsiN,Double_t &qnxV0C,Double_t &qnyV0C,Double_t &qnxV0A,Double_t &qnyV0A){
